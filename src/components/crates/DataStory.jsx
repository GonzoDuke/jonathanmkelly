// CRATES -- the data-story. Rendered by Astro WITHOUT a client directive,
// so this ships as static HTML + inline SVG (zero client JS). Every number
// comes precomputed from AGGREGATES in src/data/crates/records.js.
...
const Section = ({ kicker, title, children, note, style }) => (
  <section style={{ padding: '54px 0', borderTop: `1px solid ${LINE}`, ...style }}>
...
function HeroStats({ agg }) {
  ...
  return (
    ...
    >
      {stats.map((s) => (
        <div key={s.label} style={{ background: '#0b0b0d', padding: '26px 20px' }}>
          <div
            style={{
              fontFamily: 'var(--serif)',
        ...
        >
          ...
          <div
            style={{
              ...
              background: `linear-gradient(180deg, ${GOLD}, rgba(212,130,42,0.35))`,
...
function ArtistsLabels({ agg }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '40px 56px' }}>
      <div>
        <div style={{ fontFamily: 'var(--mono)', fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: MUT2, marginBottom: 16 }}>
...
function Formats({ agg }) {
  ...
  return (
    <div>
      ...
      {f.colorSwatches.length > 0 && (
        <div style={{ marginTop: 26 }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: MUT2, marginBottom: 14 }}>
            The color way
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '14px 20px' }}>
            {f.colorSwatches.map((s) => (
              <div key={s.name} style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                ...
                />
                <span style={{ fontFamily: 'var(--body)', fontSize: 14, color: TXT }}>{s.name}</span>
                <span style={{ fontFamily: 'var(--mono)', fontSize: 11, color: MUT }}>{s.count}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  if (!agg.genreKnownCount) {
    return (
      ...
      >
        ...
        Run <span style={{ fontFamily: 'var(--mono)', fontStyle: 'normal', fontSize: 13, color: GOLD }}>npm run records:update</span> to populate it.
      </div>
    );
  }
  const top = agg.genres.slice(0, 10);
  return (
    <div>
      <BarList items={top} />
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div style={{ fontFamily: 'var(--mono)', fontSize: 11.5, color: GOLD, marginTop: 6, letterSpacing: '0.04em' }}>
...
// -- 8. Notable ----------------------------------------------
function MiniRec({ rec, tag }) {
  if (!rec) return null;
  return (
    <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
      <div style={{ width: 64, height: 64, flex: '0 0 64px', borderRadius: 4, overflow: 'hidden', boxShadow: '0 4px 14px rgba(0,0,0,0.4)' }}>
        <CoverTile rec={rec} />
      </div>
      <div>
        <div style={{ fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', color: GOLD }}>
          {tag}
        </div>
        <div style={{ fontFamily: 'var(--serif)', fontSize: 18, color: '#fff', lineHeight: 1.15, marginTop: 4 }}>
          {rec.title}
        </div>
        <div style={{ fontFamily: 'var(--body)', fontSize: 13.5, color: MUT2, marginTop: 2 }}>
          {rec.artist}
          {rec.releasedYear ? ` · ${rec.releasedYear}` : ''}
        </div>
      </div>
    </div>
  );
...
function Notable({ agg }) {
  const nb = agg.notable;
  const quote = (nb.noteworthy || [])[0];
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 34 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: 26 }}>
        <MiniRec rec={nb.oldest} tag="Oldest pressing" />
        <MiniRec rec={nb.newest} tag="Newest release" />
        <MiniRec rec={nb.recentAddition} tag="Latest addition" />
      </div>
      {quote && (
        <figure
          style={{
        ...
        >
          ...
          <figcaption style={{ fontFamily: 'var(--mono)', fontSize: 12, color: MUT2, marginTop: 14 }}>
            {quote.artist}, {quote.title}
            {quote.releasedYear ? ` (${quote.releasedYear})` : ''}
          </figcaption>
        </figure>
      )}
    </div>
...
export default function DataStory({ agg }) {
  return (
    <div style={{ color: TXT }}>
      <Section kicker="At a glance" title="The collection, by the numbers" style={{ borderTop: 'none' }}>
        <HeroStats agg={agg} />
      </Section>
      ...
      <Section kicker="Names that recur" title="Who fills the crates">
        <ArtistsLabels agg={agg} />
      </Section>

      <Section kicker="Format flexing" title="Not just black wax">
        <Formats agg={agg} />
      </Section>

      <Section kicker="Sound" title="What it mostly sounds like">
        <Genres agg={agg} />
      </Section>

      <Section kicker="Collections within the collection" title="Whose records are whose">
        <Folders agg={agg} />
      </Section>

      <Section kicker="One of a kind" title="The standouts">
        <Notable agg={agg} />
      </Section>
    </div>
  );
